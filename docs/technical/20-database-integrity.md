...
`CottonServerSettings.TrustedProxyIpAddress` is intentionally **not** part of the signed set above. It is an operator-managed network mode: real proxy addresses are verified against the live connection peer when saved, while reserved `0.0.0.0` selects direct mode. Changing it must not create a database-integrity schema transition or require legacy rows to be resigned.
