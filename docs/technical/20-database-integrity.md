...
`CottonServerSettings.TrustedProxyIpAddress` is intentionally **not** part of the signed set above. It is an operator-managed network binding verified against the live connection peer when saved; changing it must not create a database-integrity schema transition or require legacy rows to be resigned.
