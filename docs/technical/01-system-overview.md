...
- **Startup ordering.** `RunApplicationAsync` builds the `WebApplication`, configures forwarded headers and auth hardening, maps controllers and the SPA fallback, then applies EF migrations (`ApplyMigrations<CottonDbContext>`), runs database-integrity bridge backfill (`ApplyDatabaseIntegrityBridgeBackfillAsync`), attempts auto-restore (`IDatabaseAutoRestoreService.TryRestoreIfEmptyAsync`, active when `COTTON_RESTORE_DATABASE_IF_EMPTY=true` and the DB is empty), warms `SettingsProvider`, maps the SignalR `EventHub`, and finally starts Kestrel (`app.RunAsync()`).
