...
- **Proxy client addresses.** Abuse controls resolve the client through `HttpRequest.GetTrustedClientIPAddress()`. When `TrustedProxyIpAddress` is configured, the immediate connection peer must match before forwarded client headers are accepted; otherwise the request fails. A null setting preserves legacy trust-all behavior and costs two security-checkup points. URL generation that depends on the external scheme separately uses `RequestBaseUrlHelpers` and `X-Forwarded-Proto`.
