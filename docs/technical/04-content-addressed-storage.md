...
- **Inline (opt-in):** `CreateFileRequestHandler.ValidateContentHashIfRequestedAsync` runs when `request.Validate` is set and `ComputedContentHash` is still null. It builds a blob stream from the ordered chunk hashes (`_storage.GetBlobStream(hashes, new PipelineContext { FileSizeBytes = fileManifest.SizeBytes })`, where `GetBlobStream` is the extension in `src/Cotton.Storage/Extensions/StoragePipelineExtensions.cs`), hashes it asynchronously with request cancellation (`Hasher.HashDataAsync(stream, ct)`), and on match sets `ComputedContentHash`; on mismatch it throws `BadRequestException("File content hash does not match the provided hash.")` (the file create fails).
- **Asynchronous (default):** `ComputeManifestHashesJob` (`[JobTrigger(hours: 12)]`, also triggered after file creation/content update) selects up to `MaxItemsPerRun` (1000) manifests where `ComputedContentHash == null`, excludes mismatches already reported by this process, orders by manifest id, reassembles each from `manifest.FileManifestChunks.GetChunkHashes()`, and compares. On match it sets `ComputedContentHash`; on **mismatch** it does *not* fix anything — it sends `SendUploadHashMismatchNotificationAsync` to the owner of every `NodeFile` pointing at the bad manifest, then suppresses repeat processing of that manifest until the process restarts. The job returns immediately if an upload is in progress or it is night time (`PerfTracker.IsUploading()` / `IsNightTime()`), and between items it yields 60 s when `IsPreviewGenerating()`/`IsUploading()` and 250 ms otherwise. The query and delays accept the Quartz cancellation token.
