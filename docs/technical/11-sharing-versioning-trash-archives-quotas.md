...
`DownloadTokenRetentionJob` (`src/Cotton.Server/Jobs/DownloadTokenRetentionJob.cs`) is a Quartz job annotated `[JobTrigger(days: 1)]`, i.e. a 1-day repeat interval. Its first execution per process awaits the cancellation-aware 4-minute `JobStartupDelays.WaitForDownloadTokenRetentionAsync` delay; later executions proceed immediately. It removes every `DownloadToken` whose `ExpiresAt != null && ExpiresAt <= now - 30 days` in a single `RemoveRange`/`SaveChangesAsync` (it loads all matching rows at once — it is **not** paginated; the paging behavior lives in `DownloadTokenExpirationService`, used for bulk revocation). So an expired token lingers (returning `404` to clients because of the expiry filter) for at least 30 days before physical deletion. Tokens with `ExpiresAt == null` are never swept by this job.
