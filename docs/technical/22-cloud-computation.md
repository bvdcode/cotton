...
Extracted text is limited per document to 256 MiB measured as UTF-8 bytes. Configure a positive byte count through `TextIndexing:MaxExtractedTextBytes` or the environment variable `TextIndexing__MaxExtractedTextBytes`. This is a text budget, not a source-file size limit: images and other non-text content do not consume it.
