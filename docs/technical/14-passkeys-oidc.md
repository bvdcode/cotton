...
The `auth.interactive` policy (`AuthRateLimitPolicies.Interactive`, defined in `src/Cotton.Server/Auth/AuthRateLimitPolicies.cs` and configured in `src/Cotton.Server/Extensions/EndpointRateLimitingExtensions.cs`) is a per-client-IP fixed-window limiter of **10 requests per minute** (`PermitLimit = 10`, `Window = 1 minute`, `QueueLimit = 0`); rejections return HTTP **429** (`RejectionStatusCode = StatusCodes.Status429TooManyRequests`). The partition key comes from `HttpRequest.GetTrustedClientIPAddress()`: a configured immediate proxy peer is validated before forwarded client headers are accepted. Only the two anonymous assertion endpoints are rate-limited; registration is gated behind `[Authorize]`.

> Note on rate-limit partitioning: after the immediate proxy trust check, the client partition uses `CF-Connecting-IP`, then `X-Real-IP`, then `X-Forwarded-For`, with the connection peer as the fallback. The proxy must overwrite these headers rather than append untrusted client input.
