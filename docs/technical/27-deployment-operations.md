...
| `AppVersionTracker:ReleaseCheckEnabled` | `true` | Deployment-level switch for the GitHub release check. Admins can also disable it at runtime through General settings. | `AppVersionTrackerService` |
...
- **App version tracking.** `AppVersionTrackerService` (a hosted `BackgroundService`) records the running version (`AppVersion` rows), warns on downgrades (an unsupported scenario), and — unless disabled via General settings, `AppVersionTracker:ReleaseCheckEnabled=false`, or the `Testing`/`IntegrationTests` environments — checks `https://api.github.com/repos/bvdcode/cotton/releases/latest` ~45 s after start and notifies admins (Medium priority) when a newer non-draft/non-prerelease release exists. Disabling the release check does not disable local running-version history. The image build stamps `APP_VERSION` (a `BUILD_VERSION` build arg sourced from GitVersion's SemVer in CI).
