...
- **Temporary DB-integrity/CTN2 startup guard.** After migrations and automatic restore, 0.5 validates the actual state it is about to serve. Empty databases start normally; non-empty databases require current integrity metadata on every protected row plus the durable completion marker written by 0.4.35. The check is independent of stable/development version strings. If it fails, Cotton serves the startup-blocked SPA and `GET /api/v1/startup/status` instead of starting the normal API. The `[Obsolete]` guard and marker contract are removed together after the supported upgrade window.
