...
- Behind a reverse proxy, configure `PublicBaseUrl` to the externally reachable HTTPS origin and set `TrustedProxyIpAddress` in General settings to the exact immediate proxy peer (the **Auto** action detects it, and **Verify and save** proves the request arrived from it). Client-IP-sensitive controls then accept `CF-Connecting-IP`, `X-Real-IP`, or `X-Forwarded-For` only from that peer. The proxy must overwrite client-supplied values. Request-derived URL generation separately reads `X-Forwarded-Proto` through `RequestBaseUrlHelpers`.
