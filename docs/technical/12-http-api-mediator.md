...
- **Rate limiting is client-IP-partitioned** through `HttpRequest.GetRemoteIPAddress()`, including the forwarded client address supplied by the reverse proxy. Public-share content is not endpoint-rate-limited; only failed share-token lookups consume its 60-per-minute failure budget.
