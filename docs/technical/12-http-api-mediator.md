...
| GET | `trusted-proxy-ip-address` | `GetTrustedProxyIpAddress` | Admin | Configured exact immediate proxy IP, nullable. |
| GET | `trusted-proxy-ip-address/observed` | `GetObservedProxyIpAddress` | Admin | Current connection peer used by the admin **Auto** action. |
| POST | `trusted-proxy-ip-address/verify-and-save` | `VerifyAndSaveTrustedProxyIpAddress` | Admin | Saves only when the candidate equals the current peer; mismatch returns entered/observed values with `saved: false`. |
...
These endpoint policies use `QueueLimit = 0` (excess requests are rejected immediately) and `AutoReplenishment = true`. Rejections return HTTP 429. The partition key comes from `HttpRequest.GetTrustedClientIPAddress()`. If `TrustedProxyIpAddress` is set, the connection peer must match before the resolver considers `CF-Connecting-IP`, `X-Real-IP`, or `X-Forwarded-For`; otherwise it throws. A null setting retains legacy trust-all behavior. `EndpointRateLimitingExtensions` registers the policies and enables `UseRateLimiter`; ordinary application requests are not globally rate-limited.
...
- **Rate limiting is client-IP-partitioned** through `HttpRequest.GetTrustedClientIPAddress()`, with the configured immediate-proxy check applied before any forwarded client address is trusted. Public-share content is not endpoint-rate-limited; only failed share-token lookups consume its 60-per-minute failure budget.
