...
- **Integrity signing throws on tamper.** Modifying an integrity-protected row whose stored MAC/version no longer matches its original values raises `DatabaseIntegrityException` during `SaveChanges`, aborting the transaction. Backfilled-null rows (no prior MAC/version) are signed without re-verification.
...
- *Database Integrity* — the `DatabaseIntegrityChangeSigner`, descriptor registry, MAC algorithm, read-time verifier, and the bridge backfill (`ApplyDatabaseIntegrityBridgeBackfillAsync`).
