...
Both detection responses also return informational `detectedProxyServices[]`. Cotton combines allowlisted product-specific request headers with a three-second, unauthenticated `HEAD` probe of `PublicBaseUrl` through the server's own DNS path; the browser additionally normalizes a known `Server` response header. The UI renders the merged topology from Cotton outward, for example `Cotton → nginx → Cloudflare → Internet`. Standard forwarding headers cannot uniquely distinguish nginx, Caddy, HAProxy, Apache, or Traefik. An operator who wants an exact local product label may configure that proxy to overwrite `X-Cotton-Proxy` with an allowlisted comma-separated chain in public-to-Cotton order (for example `cloudflare,traefik`). These hints never establish trust, never expose raw header values, and do not affect address validation.
