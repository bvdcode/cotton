...
{
    /// <summary>
    /// Returns the active layout and root node for the requested tree type.
    /// </summary>
    Task<(Layout Layout, Node Root)> GetLayoutAndRootAsync(Guid userId, NodeType nodeType, CancellationToken ct = default);

    /// <summary>
    /// Resolves a slash-separated node path relative to the requested root.
    /// </summary>
    Task<Node?> ResolveNodeByPathAsync(Guid userId, string? path, NodeType nodeType, CancellationToken ct = default);

    /// <summary>
    /// Resolves the parent node and final resource name for a path that may not exist yet.
    /// </summary>
    ...
    /// <summary>
    /// Builds the display path from the root node to a child node.
    /// </summary>
