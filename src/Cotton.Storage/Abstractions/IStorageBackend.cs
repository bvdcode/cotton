...
namespace Cotton.Storage.Abstractions
{
    ...
    {
        /// <summary>
        /// Removes backend-specific temporary files older than the supplied time-to-live.
        /// </summary>
        void CleanupTempFiles(TimeSpan ttl);

        /// <summary>
        /// Deletes a stored blob and returns whether it existed.
        /// </summary>
        Task<bool> DeleteAsync(string uid);

        /// <summary>
        /// Returns whether a blob with the supplied UID exists.
        /// </summary>
        Task<bool> ExistsAsync(string uid);

        /// <summary>
        /// Returns the stored blob size in bytes, or zero when the blob is missing.
        /// </summary>
        Task<long> GetSizeAsync(string uid);

        /// <summary>
        /// Opens a readable stream for a stored blob.
        /// </summary>
        Task<Stream> ReadAsync(string uid);

        /// <summary>
        /// Writes the supplied blob stream when the UID is not already present.
        /// </summary>
