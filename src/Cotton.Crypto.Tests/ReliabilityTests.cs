...
namespace Cotton.Crypto.Tests
{
    ...
    public class ReliabilityTests
    {
        ...
        {
            byte[] key = Key();
            AesGcmStreamCipher cipher = new AesGcmStreamCipher(key, keyId: 10, threads: 1);
            using MemoryStream input = new MemoryStream([]);
            using NonSeekableReadStream nonSeek = new NonSeekableReadStream(input);
        ...
        public void ChunkSize_Boundaries_Min_And_Max()
        {
            ...
            using MemoryStream enc1 = new MemoryStream();
            ...
            using MemoryStream enc2 = new MemoryStream();
        ...
        {
            byte[] key = Key();
            AesGcmStreamCipher cipher = new AesGcmStreamCipher(key, keyId: 12, threads: 1);
        ...
        {
            byte[] key = Key();
            AesGcmStreamCipher cipher = new AesGcmStreamCipher(key, keyId: 13, threads: 1, strictLengthCheck: true);
            ...
            // L-1
            SetTotal(L - 1);
            using MemoryStream tamperedMinus = new MemoryStream(bytes, writable: false);
            using MemoryStream outDec1 = new MemoryStream();
            Assert.ThrowsAsync<AuthenticationTagMismatchException>(async () => await cipher.DecryptAsync(tamperedMinus, outDec1));
            // L+1
            SetTotal(L + 1);
            using MemoryStream tamperedPlus = new MemoryStream(bytes, writable: false);
            using MemoryStream outDec2 = new MemoryStream();
            Assert.ThrowsAsync<AuthenticationTagMismatchException>(async () => await cipher.DecryptAsync(tamperedPlus, outDec2));
            // Exactly L passes
            SetTotal(L);
            using MemoryStream ok = new MemoryStream(bytes, writable: false);
            using MemoryStream outDecOk = new MemoryStream();
        ...
        {
            byte[] key = Key();
            AesGcmStreamCipher cipher = new AesGcmStreamCipher(key, keyId: 15, threads: 2);
            byte[] pattern = new byte[AesGcmStreamCipher.MinChunkSize];
            for (int i = 0; i < pattern.Length; i++) pattern[i] = 0xAA;
            using MemoryStream input = new MemoryStream(pattern);
            using MemoryStream outEnc = new MemoryStream();
            ...
            byte[] buf = System.Buffers.ArrayPool<byte>.Shared.Rent(pattern.Length);
            try
            {
                bool equal = true;
