...
- [x] Add remote folder picker or searchable remote folder selector.
  ...
  Verification 2026-06-04: the add-folder wizard now labels cloud selection explicitly as the current folder, uses `Back`/`Open` navigation instead of ambiguous glyph-only buttons, and names the primary action `Use this folder`. Focused `ShellViewModelSyncPairCommandTests` passed 22/22, full `Cotton.Sync.Desktop.Tests` passed 145/145, and `dotnet build src/Cotton.sln --configuration Release` passed with 0 warnings.
- [ ] Add settings screen.
...
- [ ] Add responsive layout for minimum window size.
  ...
  Partial 2026-06-04: setup, settings, and add-folder wizard labels were tightened (`Start at login`, `Show notifications`, `Check`, `Up`, `Sync`) and tooltips were added where shorter labels could become ambiguous; the empty-state accent glyph now keeps contrast in dark mode. Full `Cotton.Sync.Desktop.Tests` passed 146/146 and `dotnet build src/Cotton.sln --configuration Release` passed with 0 warnings. Keep unchecked until refreshed screenshots prove the minimum-size layout has no clipping or overlap.
  Partial 2026-06-04: sign-in window sizing was corrected after Xvfb screenshot QA showed the `Start at login` checkbox clipped out of the compact window. The refreshed screenshot at `/tmp/cotton-signin-current.png` shows the checkbox visible without enlarging the server step or dashboard, and the smoke log no longer contains hidden settings binding warnings from a null selected sync pair. Full `Cotton.Sync.Desktop.Tests` passed 150/150 and `dotnet build src/Cotton.sln --configuration Release` passed with 0 warnings. Keep unchecked until dashboard, settings, conflict, and error states receive the same screenshot pass.
...
- [ ] Capture screenshots for onboarding, dashboard, settings, conflict state, and error state.
  ...
  Partial 2026-06-04: refreshed current sign-in screenshot at `/tmp/cotton-signin-current.png` after tightening labels and resizing the sign-in window; the screen uses `app.cottoncloud.dev`, shows normalized HTTPS server verification, and includes the `Start at login` checkbox without clipping. Keep unchecked until dashboard, settings, conflict, and error screenshots are refreshed.
- [ ] Review screenshots for clipping, overlap, weak hierarchy, and inconsistent spacing.
  ...
  Partial 2026-06-04: reviewed `/tmp/cotton-signin-current.png`; the sign-in window remains centered and compact, credential fields fit, `Start at login` is visible, and no binding warnings were written to the Xvfb smoke log. Keep unchecked until the dashboard/settings/conflict/error screenshot matrix is reviewed.
...
- [ ] Add notification adapter.
  ...
  Partial 2026-06-04: Windows notification adapter support was added through an isolated `WindowsToastNotificationService` that invokes the native Windows Toast notification API from a PowerShell encoded command, while Linux continues to use the existing `notify-send` adapter. `DesktopNotificationServiceFactory` now selects the Windows adapter when `powershell.exe` or `pwsh.exe` is available. Focused notification platform tests passed 7/7, full `Cotton.Sync.Desktop.Tests` passed 150/150, and `dotnet build src/Cotton.sln --configuration Release` passed with 0 warnings. Keep unchecked until native notification behavior is manually verified in real Windows and Linux desktop sessions.
