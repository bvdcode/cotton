...
- [ ] Add empty-directory reconciliation.
  ...
  Partial 2026-06-04: `SyncEngine` now reconciles no-baseline directories before files, creating local folders for remote-only directories and creating remote folders for local-only directories in parent-first order. Desktop composition passes `SdkRemoteDirectorySynchronizer` into the sync engine. Focused directory create tests passed 2/2, full `Cotton.Sync.Tests` passed 104/104, full `Cotton.Sync.Desktop.Tests` passed 145/145, and `dotnet build src/Cotton.sln --configuration Release` passed with 0 warnings. Keep unchecked until directory delete/move/rename and guarded baseline deletion are covered.
  ...
  Partial 2026-06-04: local and remote empty directory renames are covered as full-mirror create-new-folder plus delete-old-empty-folder reconciliation, preserving the new baseline and removing the old one. Focused rename tests passed 2/2, full `Cotton.Sync.Tests` passed 114/114, and `dotnet build src/Cotton.sln --configuration Release` passed with 0 warnings. Keep unchecked until empty-directory move cases are explicitly covered.
