...
- Checksums are generated by the desktop publish target for publish outputs and by the release workflow for uploaded release assets; final release artifact checksum verification must be rerun before release.
...
- Desktop tests have passed locally, most recently `Cotton.Sync.Desktop.Tests` 218/218.
- Desktop packaging metadata tests cover publish profiles, app icon metadata, Linux `.desktop` metadata, `.deb` packaging script, Linux package smoke wiring, Linux `.deb` install/upgrade smoke wiring, Windows CI smoke, Windows `.zip` artifact upload/smoke, Windows installer script/install/upgrade smoke wiring, and release artifact checksum generation.
...
- CI contains Linux publish self-test, Linux archive/deb extraction self-tests, Linux `.deb` install/uninstall/upgrade self-tests with Secret Service setup, Windows apphost self-test, Windows zip extraction self-test, Windows installer install/uninstall/upgrade self-tests, and Windows installer build/upload on `windows-latest`.
