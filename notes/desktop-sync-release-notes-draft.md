...
- Checksums are generated by the desktop publish target for publish outputs; final release artifact checksum verification must be rerun before release.
...
- Desktop tests have passed locally, most recently `Cotton.Sync.Desktop.Tests` 209/209.
- Desktop packaging metadata tests cover publish profiles, app icon metadata, Linux `.desktop` metadata, `.deb` packaging script, Windows CI smoke, and Windows `.zip` artifact upload.
...
- CI contains a Linux self-test smoke with Secret Service setup and a Windows apphost self-test smoke on `windows-latest`.
...
- Final release branch diff review and final checksum generation for uploaded artifacts.
