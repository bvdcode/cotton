...
- Desktop tests have passed locally, most recently `Cotton.Sync.Desktop.Tests` 214/214.
- Desktop packaging metadata tests cover publish profiles, app icon metadata, Linux `.desktop` metadata, `.deb` packaging script, Linux package smoke wiring, Windows CI smoke, Windows `.zip` artifact upload/smoke, Windows installer script/CI wiring, and release artifact checksum generation.
...
- CI contains Linux publish self-test, Linux archive/deb extraction self-tests with Secret Service setup, Windows apphost self-test, Windows zip extraction self-test, and Windows installer build/upload on `windows-latest`.
