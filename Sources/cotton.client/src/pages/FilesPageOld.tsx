...
const FilesPage = () => {
  ...
  const onUpload = async () => {
    ...
    try {
      ...
      const concurrency =
        selectedFile.size > 1_000_000_000 ? 1 : UPLOAD_CONCURRENCY_DEFAULT;
      let next = 0;
      let active = 0;
        ...
        <Box sx={{ flex: 1, display: "flex", alignItems: "center", ml: 2 }}>
          ...
          >
            {(() => {
              ...
              if (pathIds.length === 0 && currentNode?.id)
              ...
              return pathIds.map((id, idx) => {
                ...
                const name =
                  nodesDict[id]?.name ||
                  (isRoot ? "Default" : id.substring(0, 8));
                if (isLast) {
                  // Last item: text only
                  return (
          ...
          <IconButton
            ...
            onClick={() => {
              ...
              const pid =
                ...
                (currentNode?.id ? parents[currentNode.id] ?? null : null);
              if (pid) {
