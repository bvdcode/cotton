...
def create_advanced_plots(encrypt_data, decrypt_data):
    ...
    fig.suptitle('Complete Performance Analysis: Encryption/Decryption Throughput', fontsize=18, fontweight='bold')
    ...
    encrypt_best = encrypt_data.groupby(['Threads', 'ChunkMB'])['Throughput'].max().reset_index()
    decrypt_best = decrypt_data.groupby(['Threads', 'ChunkMB'])['Throughput'].max().reset_index()
...
def print_analysis_summary(encrypt_data, decrypt_data, encrypt_optimal, decrypt_optimal):
    ...
    print(f"      Потоков: {encrypt_optimal['Threads']}, Размер чанка: {encrypt_optimal['ChunkMB']}MB")
    
    print(f"   Decryption:")
    print(f"      Лучший результат: {decrypt_optimal['Throughput']:.1f} MB/s")
    print(f"      Потоков: {decrypt_optimal['Threads']}, Размер чанка: {decrypt_optimal['ChunkMB']}MB")
    ...
    print(f"      Мин/Макс: {encrypt_data['Throughput'].min():.1f} / {encrypt_data['Throughput'].max():.1f} MB/s")
    
    print(f"   Decryption:")
    print(f"      Среднее: {decrypt_data['Throughput'].mean():.1f} MB/s")
    print(f"      Медиана: {decrypt_data['Throughput'].median():.1f} MB/s")
    print(f"      Мин/Макс: {decrypt_data['Throughput'].min():.1f} / {decrypt_data['Throughput'].max():.1f} MB/s")
    ...
    for operation, data in [("Encryption", encrypt_data), ("Decryption", decrypt_data)]:
        ...
        print(f"   {operation}:")
        print(f"      Лучший размер чанка: {best_chunk}MB ({chunk_performance[best_chunk]:.1f} MB/s)")
        print(f"      Худший размер чанка: {worst_chunk}MB ({chunk_performance[worst_chunk]:.1f} MB/s)")
